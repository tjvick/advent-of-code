...
@cache
def distance_from_end(position, end_position):
    return abs(end_position[0] - position[0]) + abs(end_position[1] - position[1])


def get_min_time_from_a_to_b(a, b, starting_time, starting_min_time):
    visits = set()
    min_time = starting_min_time
    paths = [Path(last_point=a, time=starting_time)]
    while len(paths):
        # print(paths)
        path = paths.pop()
        ...
        if path.time + distance_from_end(path.last_point, b) >= min_time:
            continue

        if path.last_point == b:
...
leg_1 = get_min_time_from_a_to_b(starting_point, ending_point, 0, 300)
print("Leg 1:", leg_1)
leg_2 = get_min_time_from_a_to_b(ending_point, starting_point, leg_1, 600)
print("Leg 2:", leg_2)
leg_3 = get_min_time_from_a_to_b(starting_point, ending_point, leg_2, 900)
print("Leg 3:", leg_3)