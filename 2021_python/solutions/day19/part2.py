...
def manhattan_distance(scanner_a, scanner_b):
    return np.sum(np.abs(scanner_a.origin - scanner_b.origin))


max_manhattan_distance = 0
for ix1 in range(n_scanners):
    for ix2 in range(ix1+1, n_scanners):
        max_manhattan_distance = max(max_manhattan_distance, manhattan_distance(scanners[ix1], scanners[ix2]))
