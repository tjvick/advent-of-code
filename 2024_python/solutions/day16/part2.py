...
while len(paths) > 0:
    ...
    if current_score > min_scores_by_location[past_locations[-1]] + 1000:
        # print("thrown out", current_location, current_direction, current_score, min_scores_by_location[past_locations[-1]])
        continue
    ...
    for next_direction in next_directions:
        ...
        if maze_map[*next_location] == '.':
            ...
            if total_score == min_score:
                print("ANOTHER BEST PATH!")
                tiles_on_best_paths.update(past_locations + [tuple(next_location)])
